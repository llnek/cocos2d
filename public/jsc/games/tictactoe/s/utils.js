...
xbox = /** @lends xbox# */{
  ...
  mapGridPos: function mapGridPos(gsz, scale) {
    ...
    var sp = _ccsx2.default.createSprite('z.png'),
        csz = cc.size(sp.getContentSize().width * scale, sp.getContentSize().height * scale),
        cells = gsz * gsz,
        ro = 8 / 72 * scale,
        gh = csz.height * ro,
        gw = csz.width * ro,
   ...
   * @private
   */
  pkFlip: function pkFlip(img, flip) {
    if (flip) {
      return img + ".i.png";
    } else {
      return img + ".png";
   ...
   * @private
   */
  xrefImg: function xrefImg(value) {
    switch (value) {
      case csts.CV_X:
        return 'x';
      case csts.CV_O:
   ...
   * @return {cc.Sprite}
   */
  drawSymbol: function drawSymbol(view, x, y, value, flip) {
    var frame = this.pkFlip(this.xrefImg(value), flip),
        s1 = _ccsx2.default.createSprite(frame);
    s1.setAnchorPoint(_ccsx2.default.acs.Center);
    s1.setPosition(x, y);
