...
var _l10njs = require("eligrey/l10njs");

var _l10njs2 = _interopRequireDefault(_l10njs);
...
ComObj = {
  ...
  hurt: function hurt(damage, from) {
    this.HP -= _skaro2.default.isnum(damage) ? damage : 1;
  },
  ...
  inflate: function inflate(options) {
    options = options || {};
    if (!!this.sprite) {
      if (_skaro2.default.hasKey(options, 'x') && _skaro2.default.hasKey(options, 'y')) {
        this.sprite.setPosition(options.x, options.y);
      }
      if (_skaro2.default.hasKey(options, 'deg')) {
        this.sprite.setRotation(options.deg);
      }
      if (_skaro2.default.hasKey(options, 'scale')) {
   ...
   * @private
   */
  ctor: function ctor(sprite, health, score) {
 ...
 * @class SimpleComp
 */
SimpleComp = _ashJs2.default.Class.extend(_skaro2.default.mergeEx(ComObj, {
...
Ashley = {
  ...
  compDef: function compDef(proto) {
    return _ashJs2.default.Class.extend(_skaro2.default.mergeEx(ComObj, proto));
  },
   ...
   * @return {Ash.Node}
   */
  nodeDef: function nodeDef(proto) {
    return _ashJs2.default.Node.create(proto);
  },
   ...
   * @return {Ash.System}
   */
  sysDef: function sysDef(proto, cps) {
    var rc = _ashJs2.default.System.extend(proto);
    _skaro2.default.eachObj(function (v, k) {
      rc[k] = v;
    }, cps || {});
    return rc;
  },
   ...
   * @return {Ash.Class}
   */
  casDef: function casDef(proto, cps) {
    var rc = _ashJs2.default.Class.extend(proto);
    _skaro2.default.eachObj(function (v, k) {
      rc[k] = v;
    }, cps || {});
    return rc;
  ...
  newEntity: function newEntity() {
    return new _ashJs2.default.Entity();
  },
   ...
   * @return {SimpleComp}
   */
  newObject: function newObject(sprite) {
...
var xbox = /** @lends xbox# */{
   ...
   * @param {Object} table
   */
  l10nInit: function l10nInit(table) {
    _l10njs2.default.toLocaleString(table || this.xcfg.l10nTable);
    _l10njs2.default.locale = cc.sys.language;
    _l10njs2.default.defaultLocale = "en";
    _skaro2.default.loggr.info("Loaded l10n strings.  locale = " + _l10njs2.default.locale);
  },
   ...
   */
  l10n: function l10n(s, pms) {
    var t = s.toLocaleString();
    return _skaro2.default.isobj(pms) ? _mustache2.default.render(t, pms) : t;
  },
   ...
   * @return {Boolean}
   */
  isIntersect: function isIntersect(a1, a2) {
   ...
   * @return {Boolean}
   */
  outOfBound: function outOfBound(a, B) {
    if (a && B) {
   ...
   */
  calcXY: function calcXY(angle, hypot) {
    // quadrants =  4 | 1
    //             --------
    //              3 | 2
    var theta = void 0,
   ...
   * @return {Number}
   */
  normalizeDeg: function normalizeDeg(deg) {
   ...
   * @return {String} path
   */
  getPList: function getPList(key) {
   ...
   * @return {String} path
   */
  getSfx: function getSfx(key) {
    return [this.fixUrl(this.xcfg.assets.sounds[key]), '.', this.xcfg.game.sfx].join('');
  },
   ...
   * @return {String} path
   */
  getSprite: function getSprite(key) {
    //cc.log('getSprite for key : ' + key);
   ...
   * @return {String} path
   */
  getTiles: function getTiles(key) {
   ...
   */
  toggleSfx: function toggleSfx(override) {
    this.xcfg.sound.open = _skaro2.default.echt(override) ? override : !this.xcfg.sound.open;
  },
   ...
   * @param {Object} options
   */
  sfxPlayMusic: function sfxPlayMusic(key, options) {
    if (this.xcfg.sound.open) {
      options = options || {};
      this.sfxMusicVol(options.vol);
      cc.audioEngine.playMusic(this.getSfx(key), options.repeat === true);
    }
  },
   ...
   * @param {Object} options
   */
  sfxPlay: function sfxPlay(key, options) {
    if (this.xcfg.sound.open) {
      options = options || {};
      this.sfxMusicVol(options.vol);
      cc.audioEngine.playEffect(this.getSfx(key), options.repeat === true);
    }
  },
   ...
   * @method sfxMusicVol
   */
  sfxMusicVol: function sfxMusicVol(v) {
    if (this.xcfg.sound.open && _skaro2.default.isnum(v)) {
  ...
  sanitizeUrl: function sanitizeUrl(url) {
    _skaro2.default.loggr.debug('About to sanitize url: ' + url);
    var rc = void 0;
    if (cc.sys.isNative) {
    ...
    _skaro2.default.loggr.debug('Sanitized url: ' + rc);
    return rc;
  },
   ...
   * @return {String} path
   */
  sanitizeUrlForDevice: function sanitizeUrlForDevice(url) {
    if (url.match(/^res\//)) {
    ...
    return _mustache2.default.render(url, {
   ...
   * @return {String}
   */
  sanitizeUrlForWeb: function sanitizeUrlForWeb(url) {
    //ensure we tell mustache not to escape html
    if (url.match(/^game/)) {
      url = '{{{gamesource-ref}}}/' + url;
    } else if (url.match(/^res/)) {
      url = '{{{media-ref}}}/' + url;
    }
    return _mustache2.default.render(url, {
      'border-tiles': this.xcfg.game.borderTiles,
  ...
  throttle: function throttle(func, wait, options) {
    ...
    var later = function later() {
      previous = options.leading === false ? 0 : _skaro2.default.now();
      timeout = null;
...
if (!!cc) {
  _skaro2.default.logger = cc;
  _skaro2.default.loggr = cc;
  _skaro2.default.loggr.info = cc.log;
  _skaro2.default.loggr.debug = cc.log;
  _skaro2.default.loggr.info('Monkey patched skaro#loggr to cc.log');
}
