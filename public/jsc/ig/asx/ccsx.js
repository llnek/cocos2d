...
var xbox = /** @lends xbox# */{
  ...
  pointInBox: function pointInBox(box, x, y) {
    var px = void 0,
        py = void 0;
    if (!sjs.echt(y)) {
      px = x.x;
   ...
   * @return {Boolean}
   */
  collide0: function collide0(spriteA, spriteB) {
    return !!spriteA && !!spriteB ? cc.rectIntersectsRect(this.bbox(spriteA), this.bbox(spriteB)) : false;
  },
   ...
   * @param {Number} pcy
   */
  setdr: function setdr(landscape, w, h, pcy) {
    if (landscape) {
   ...
   * @return {Boolean}
   */
  outOfBound: function outOfBound(ent, B) {
    return !!ent ? _asterix2.default.outOfBound(this.bbox4(ent.sprite), B || this.vbox()) : false;
  },
   ...
   * @return null
   */
  undoTimer: function undoTimer(par, tm) {
   ...
   */
  createTimer: function createTimer(par, tm) {
   ...
   * @return {Boolean}
   */
  timerDone: function timerDone(t) {
   ...
   * @return {cc.Sprite}
   */
  createSprite: function createSprite(name) {
   ...
   * @param delay
   */
  runScene: function runScene(ns, delay) {
    delay = delay || 0.6;
   ...
   * @return {cc.rect} rect
   */
  bbox: function bbox(sprite) {
    return cc.rect(this.getLeft(sprite), this.getBottom(sprite), this.getWidth(sprite), this.getHeight(sprite));
  },
   ...
   * @return {Number}
   */
  getScaledHeight: function getScaledHeight(sprite) {
   ...
   * @return {Number}
   */
  getHeight: function getHeight(sprite) {
   ...
   * @return {Number}
   */
  getScaledWidth: function getScaledWidth(sprite) {
   ...
   * @return {Number}
   */
  getWidth: function getWidth(sprite) {
   ...
   * @return {Number}
   */
  getLeft: function getLeft(sprite) {
   ...
   * @return {Number}
   */
  getRight: function getRight(sprite) {
    return sprite.getPosition().x + this.getWidth(sprite) * 0.5;
  },
   ...
   * @return {Number}
   */
  getBottom: function getBottom(sprite) {
    return sprite.getPosition().y - this.getHeight(sprite) * 0.5;
  },
   ...
   * @return {Number}
   */
  getLastLeft: function getLastLeft(ent) {
   ...
   * @return {Number}
   */
  getLastRight: function getLastRight(ent) {
    if (sjs.echt(ent.lastPos)) {
      return ent.lastPos.x + this.getWidth(ent.sprite) * 0.5;
    } else {
   ...
   * @return {Number}
   */
  getLastTop: function getLastTop(ent) {
    if (sjs.echt(ent.lastPos)) {
   ...
   * @return {Number}
   */
  getLastBottom: function getLastBottom(ent) {
    if (sjs.echt(ent.lastPos)) {
      return ent.lastPos.y - this.getHeight(ent.sprite) * 0.5;
    } else {
  ...
  vrect: function vrect() {
    var vr = cc.view.getViewPortRect(),
  ...
  screen: function screen() {
    return cc.sys.isNative ? cc.view.getFrameSize() : cc.director.getWinSize();
  },
   ...
   * @return {cc.Point}
   */
  vboxMID: function vboxMID(box) {
    return cc.p(box.left + (box.right - box.left) * 0.5, box.bottom + (box.top - box.bottom) * 0.5);
  },
   ...
   * @return {Object}
   */
  traceEnclosure: function traceEnclosure(dt, bbox, rect, vel) {
   ...
   * @return {cc.Sprite}
   */
  getSprite: function getSprite(frameid) {
   ...
   * @method onKeyPolls
   */
  onKeyPolls: function onKeyPolls(kb) {
    if (!this.hasKeyPad()) {
      return;
    }
    cc.eventManager.addListener({
      onKeyPressed: function onKeyPressed(key, e) {
        kb[key] = true;
      },
      onKeyReleased: function onKeyReleased(key, e) {
        kb[key] = false;
      },

      event: cc.EventListener.KEYBOARD
    }, _asterix2.default.main);
  },
   ...
   * @return {Boolean}
   */
  hasMouse: function hasMouse() {
    return !!cc.sys.capabilities['mouse'];
  },
  onMouse: function onMouse(bus) {
    if (!this.hasMouse()) {
      return;
    }
    cc.eventManager.addListener({
  ...
  onTouchAll: function onTouchAll(bus) {
    if (!this.hasTouch()) {
      return;
    }
    cc.eventManager.addListener({
      ...
      prevTouchId: -1,
      onTouchesBegan: function onTouchesBegan(ts, e) {
        return true;
      },
      onTouchesEnded: function onTouchesEnded(ts, e) {
        bus.fire('/touch/all/end', { group: 'touch',
          event: e,
          loc: ts[0].getLocation() });
      },
      onTouchesMoved: function onTouchesMoved(ts, e) {
        var id = ts[0].id;
  ...
  onTouchOne: function onTouchOne(bus) {
    if (!this.hasTouch()) {
      return;
    }
    cc.eventManager.addListener({
      ...
      swallowTouches: true,
      onTouchBegan: function onTouchBegan(t, e) {
        return true;
      },
      onTouchMoved: function onTouchMoved(t, e) {
        bus.fire('/touch/one/move', { group: 'touch',
          event: e,
          delta: t.getDelta(),
          loc: t.getLocation() });
      },
      onTouchEnded: function onTouchEnded(t, e) {
   ...
   * @private
   */
  resolveElastic: function resolveElastic(obj1, obj2) {
   ...
   * @return {cc.Menu}
   */
  tmenu: function tmenu(items, scale) {
    ...
    for (var n = 0; n < items.length; ++n) {
      obj = items[n];
      mi = new cc.MenuItemLabel(new cc.LabelBMFont(obj.text, obj.fontPath), obj.selector || obj.cb, obj.target);
   ...
   * @return {cc.Menu}
   */
  tmenu1: function tmenu1(options) {
    var menu = this.tmenu(options);
    if (options.anchor) {
      menu.setAnchorPoint(options.anchor);
    }
    if (options.pos) {
   ...
   * @return {cc.Menu}
   */
  vmenu: function vmenu(items, options) {
    var hint = options || {},
        m = this.pmenu(true, items, hint.scale, hint.padding);
    if (!!hint.pos) {
   ...
   * @return {cc.Menu}
   */
  hmenu: function hmenu(items, options) {
    var hint = options || {},
        m = this.pmenu(false, items, hint.scale, hint.padding);
    if (!!hint.pos) {
      m.setPosition(hint.pos);
    }
    return m;
   ...
   * @return {cc.Menu}
   */
  pmenu: function pmenu(vertical, items, scale, padding) {
    var menu = new cc.Menu(),
        obj = void 0,
        mi = void 0,
        t = 0;
    ...
    for (var n = 0; n < items.length; ++n) {
      obj = items[n];
      mi = new cc.MenuItemSprite(new cc.Sprite(obj.nnn), new cc.Sprite(obj.sss || obj.nnn), new cc.Sprite(obj.ddd || obj.nnn), obj.selector || obj.cb, obj.target);
      if (!!obj.color) {
   ...
   * @return {cc.Menu}
   */
  pmenu1: function pmenu1(options) {
